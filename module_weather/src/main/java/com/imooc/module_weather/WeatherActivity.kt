                        ...
                        if (response.isSuccessful){
                            response.body()?.let {
                                ...
                                it.result.realtime.apply {
                                    //set weather sunny/cloudy
                                    mInfo.text = info
                                    //set pic
                                    mIvWid.setImageResource(WeatherIconTools.getIcon(wid))
                                    //set temp
                                    mTemperature.text = String.format("%s%s", temperature, getString(R.string.app_weather_t))
                                    //set humidity
                                    mHumidity.text = String.format("%s\t%s", getString(R.string.app_weather_humidity), humidity)
                                    //set wind direction
                                    mDirect.text = String.format("%s\t%s", getString(R.string.app_weather_direct), direct)
                                    //set wind power
                                    mPower.text = String.format("%s\t%s", getString(R.string.app_weather_power), power)
                                    //set aqi
                                    mAqi.text = String.format("%s\t%s", getString(R.string.app_weather_aqi), aqi)
                                }
