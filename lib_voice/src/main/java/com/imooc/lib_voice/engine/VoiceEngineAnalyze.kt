                            ...
                            userAppName?.let { appName ->
                                if (appName.length() > 0){
                                    ...
                                    when (intent) {
                                        ...
                                        else -> mOnNluResultListener.otherApp(word) //其他App操作
                                    }
                                }else{
                ...
                NluWords.NLU_CONSTELL -> {
                    ...
                    consTellNameArray?.let { consTell ->
                        if (consTell.length() > 0){
                            ...
                            val consTellName = slots.optJSONObject("user_constell_name")
                            when (intent) {
                                NluWords.INTENT_CONSTELL_TIME -> mOnNluResultListener.conTellTime(word)
                                NluWords.INTENT_CONSTELL_INFO -> mOnNluResultListener.conTellInfo(word)
                                else -> mOnNluResultListener
